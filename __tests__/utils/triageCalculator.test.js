 ...
 * 檢傷級別計算測試
 * 
 * 這個測試文件演示如何為核心業務邏輯編寫單元測試
 * 
 * 要運行測試，請執行：
 * npm test
 * 
 * 注意：這些測試需要安裝 Jest 和相關依賴
 * 安裝指令：npm install --save-dev jest @testing-library/react @testing-library/jest-dom
 */

import { calculateTriageLevel, safeParseInt } from '../../utils/triageCalculator';
...
describe('calculateTriageLevel', () => {
  describe('Level 1 - 復甦救援', () => {
    test('SpO2 < 80 應該返回 Level 1', () => {
      const vitals = { spo2: '75', sbp: '', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });

    test('SpO2 = 79 應該返回 Level 1', () => {
      const vitals = { spo2: '79', sbp: '', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });

    test('GCS ≤ 8 應該返回 Level 1', () => {
      const vitals = { spo2: '', sbp: '', hr: '', gcs: '8' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });

    test('GCS = 5 應該返回 Level 1', () => {
      const vitals = { spo2: '', sbp: '', hr: '', gcs: '5' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });

    test('SpO2 < 80 且 GCS ≤ 8 應該返回 Level 1', () => {
      const vitals = { spo2: '70', sbp: '', hr: '', gcs: '7' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });
  });
  ...
  describe('Level 2 - 危急', () => {
    ...
    test('SBP > 220 應該返回 Level 2', () => {
      const vitals = { spo2: '', sbp: '230', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(2);
    });

    test('HR > 150 應該返回 Level 2', () => {
      const vitals = { spo2: '', sbp: '', hr: '160', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(2);
    });

    test('HR < 40 應該返回 Level 2', () => {
      const vitals = { spo2: '', sbp: '', hr: '35', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(2);
    });
  });
  ...
  describe('Level 4 - 次緊急', () => {
    test('有 SBP 但不符合更高級別應該返回 Level 4', () => {
      const vitals = { spo2: '', sbp: '120', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(4);
    });

    test('有 HR 但不符合更高級別應該返回 Level 4', () => {
      const vitals = { spo2: '', sbp: '', hr: '80', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(4);
    });

    test('SpO2 ≥ 95 且有其他生命徵象應該返回 Level 4', () => {
      const vitals = { spo2: '98', sbp: '120', hr: '75', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(4);
    });
  });
  ...
  describe('Level 5 - 非緊急', () => {
    test('所有生命徵象為空應該返回 Level 5', () => {
      const vitals = { spo2: '', sbp: '', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(5);
    });

    test('只有 GCS 且為正常值應該返回 Level 5', () => {
      const vitals = { spo2: '', sbp: '', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(5);
    });
  });
  ...
  describe('邊界情況測試', () => {
    ...
    test('應該處理無效的字串輸入', () => {
      const vitals = { spo2: 'abc', sbp: 'def', hr: 'ghi', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(5);
    });

    test('應該處理 null 值', () => {
      const vitals = { spo2: null, sbp: null, hr: null, gcs: null };
      expect(calculateTriageLevel(vitals)).toBe(5);
    });

    test('應該處理混合的有效和無效輸入', () => {
      const vitals = { spo2: '75', sbp: 'invalid', hr: '', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });
  });
  ...
  describe('優先級測試', () => {
    test('當同時滿足 Level 1 和 Level 2 條件時，應該返回 Level 1', () => {
      const vitals = { spo2: '75', sbp: '250', hr: '160', gcs: '7' };
      expect(calculateTriageLevel(vitals)).toBe(1);
    });

    test('當同時滿足 Level 2 和 Level 3 條件時，應該返回 Level 2', () => {
      const vitals = { spo2: '85', sbp: '190', hr: '125', gcs: '15' };
      expect(calculateTriageLevel(vitals)).toBe(2);
    });

    test('當同時滿足 Level 3 和 Level 4 條件時，應該返回 Level 3', () => {
      const vitals = { spo2: '92', sbp: '150', hr: '100', gcs: '15' };
